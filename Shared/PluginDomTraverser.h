#ifndef _PLUGIN_TRAVERSER_H_
#define _PLUGIN_TRAVERSER_H_


#include "AdPluginTypedef.h"


class CPluginDomTraverserCache
{
public:

    long m_elements;

    CPluginDomTraverserCache() : m_elements(0) {};
...
template <class T>
CPluginDomTraverser<T>::CPluginDomTraverser() : 
	m_isHeaderTraversed(false), m_cacheDomElementCount(0), m_cacheIndexLast(0), m_cacheElementsMax(5000)
{
...
template <class T>
void CPluginDomTraverser<T>::TraverseHeader(bool isHeaderTraversed)
{
	m_isHeaderTraversed = isHeaderTraversed;
}

template <class T>
void CPluginDomTraverser<T>::TraverseDocument(IWebBrowser2* pBrowser, const CString& domain, const CString& documentName)
{
	m_domain = domain;
...
template <class T>
void CPluginDomTraverser<T>::TraverseSubdocument(IWebBrowser2* pBrowser, const CString& domain, const CString& documentName)
{
	m_domain = domain;
...
template <class T>
bool CPluginDomTraverser<T>::IsEnabled()
{
	CPluginClient* client = CPluginClientFactory::GetLazyClientInstance();

	return client && CPluginSettings::GetInstance()->IsPluginEnabled();
}


template <class T>
void CPluginDomTraverser<T>::TraverseDocument(IWebBrowser2* pBrowser, bool isMainDoc, CString indent)
{
	if (!IsEnabled()) return;
...
template <class T>
void CPluginDomTraverser<T>::TraverseChild(IHTMLElement* pEl, IWebBrowser2* pBrowser, CString& indent, bool isCached)
{
    int  cacheIndex = -1;
...
template <class T>
void CPluginDomTraverser<T>::ClearCache()
{
    m_criticalSection.Lock();
    {
        m_cacheIndexLast = 0;
