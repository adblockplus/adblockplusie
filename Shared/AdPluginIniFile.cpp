...
CAdPluginIniFile::CAdPluginIniFile(const CStringA& filename, bool hasChecksum) : 
...
void CAdPluginIniFile::SetInitialChecksumString(const CStringA& str)
{
	m_checksumInit = str;
}

CStringA CAdPluginIniFile::GetFilePath() const
{
    return m_filename;
}


void CAdPluginIniFile::Clear()
{
    m_data.clear();
...
bool CAdPluginIniFile::ReadString(const CStringA& memFile)
{
    bool isOk = true;
    ...
    CAdPluginIniFile::TSectionData sectionData;
...
bool CAdPluginIniFile::Write()
{
    ...
    if (m_isDirty)
    {
        ...
        else
        {
            ...
            for (CAdPluginIniFile::TData::iterator it = m_data.begin(); it != m_data.end(); ++it)
            {
                line += "[" + it->first + "]\r\n";

                m_checksum->Add(it->first);

                for (CAdPluginIniFile::TSectionData::iterator itValues = it->second.begin(); itValues != it->second.end(); ++itValues)
                {
...
CStringA CAdPluginIniFile::GetValue(const CStringA& section, const CStringA& key) const
{
    CStringA value;
...
bool CAdPluginIniFile::SetValue(const CStringA& section, const CStringA& key, const CStringA& value)
{
    bool isSet = false;
    
    CAdPluginIniFile::TData::iterator it = m_data.find(section);
    if (it != m_data.end())
    {
        CAdPluginIniFile::TSectionData::iterator itValue = it->second.find(key);
...
// Used to add or set a key value pair to a section
bool CAdPluginIniFile::SetValue(const CStringA& section, const CStringA& key, int value)
{
	CStringA valueStr;
...
bool CAdPluginIniFile::HasSection(const CStringA& section) const
{
...
bool CAdPluginIniFile::HasKey(const CStringA& section, const CStringA& key) const
{
    return !GetValue(section, key).IsEmpty();
}


void CAdPluginIniFile::UpdateSection(const CStringA& section, const CAdPluginIniFile::TSectionData& data)
{
    m_data[section] = data;
    m_isDirty = true;
}
...
const CAdPluginIniFile::TSectionNames& CAdPluginIniFile::GetSectionNames() const
{
...
CAdPluginIniFile::TSectionData CAdPluginIniFile::GetSectionData(const CStringA& section) const
{
    CAdPluginIniFile::TSectionData sectionData;
    
    CAdPluginIniFile::TData::const_iterator it = m_data.find(section);
    if (it != m_data.end())
    {
...
bool CAdPluginIniFile::IsValidChecksum() const
{
    return m_isValidChecksum && m_hasChecksum || !m_hasChecksum;
}

unsigned int CAdPluginIniFile::GetLastError() const
{
    return m_lastError;
}